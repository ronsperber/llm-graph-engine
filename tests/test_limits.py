...
def test_max_node_visits_error():
    ...
    node = FunctionalNode(func=lambda s: s, name="A", next_node_name="A")

    runner = GraphRunner(
        nodes=[node], start_node="A", max_node_visits=1, on_max_visits="error"
    )
...
def test_visit_tracker_reset():
    ...
    node = FunctionalNode(func=lambda s: s, name="A", next_node_name=None)

    runner = GraphRunner(nodes=[node], start_node="A", max_node_visits=1)
